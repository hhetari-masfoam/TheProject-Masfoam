...
Public Class CostCorrectionService_Old
...
INNER JOIN Production_Header p
    ON p.StockTransactionID = h.TransactionID
...
", con)
    ...
    End Function
    Public Function GetTransactionDetailsByDetailIDs_CUT(detailIDs As List(Of Long)) As DataTable
...
SELECT 
    ...
    ISNULL(l.InQty,0) AS InQty,
    ISNULL(l.OutQty,0) AS OutQty
...
LEFT JOIN (
    ...
    ON l.SourceDetailID = d.DetailID

WHERE 
    d.SourceDocumentDetailID IN (" & ids & ")
    OR d.SourceDocumentDetailID = (
        SELECT DISTINCT CutID 
        FROM Production_CuttingOutput 
        WHERE CutOutputID IN (" & ids & ")
    )
"
