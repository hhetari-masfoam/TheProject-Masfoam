...
Public Class frmCostCorrection_old
    ...
    Private _revaluationService As CostCorrectionService_Old
    ...
    '==============================
    ' Cost Correction Context
    '==============================
    Private _referenceTransactionID As Integer = 0
...
"
        ...
        If rbtnEditMode.Checked Then
            ...
            If CurrentDocumentID > 0 Then
                ...
                Select Case CurrentOperationType
                    ...
                    Case "SCR"
                        LoadWasteDocumentForRevaluation(CurrentDocumentID)
    ...
    Private Sub frmRevaluation_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            IsCancelMode = False
            _revaluationService = New CostCorrectionService_Old(ConnStr)
    ...
    Private Sub tabMain_SelectedIndexChanged(sender As Object, e As EventArgs) Handles tabMain.SelectedIndexChanged
        ...
        ElseIf tabMain.SelectedTab.Name = "tabPRT" Then
            dgvMain.Parent = tabPRT
            Panel3.Parent = tabPRT
    ...
    e As EventArgs
        ...
        Select Case tabName
            ...
            Case "tabSRT"
                ...
                End Using
            Case "tabPRT"
                Using f As New frmPurchaseReturnsearch()
                    If f.ShowDialog() <> DialogResult.OK Then Exit Sub
                    If f.SelectedDocumentID <= 0 Then Exit Sub
                    LoadOriginalDocumentForRevaluation(f.SelectedDocumentID)
                    SetModeUI()
            ...
            Case "tabSCR", "tabScrap"
                Using f As New frmCuttingWasteCalculatorSearch()
                    ...
                    LoadWasteDocumentForRevaluation(f.SelectedWasteID)
                    SetModeUI()
                End Using
            Case "tabLoading"
                Using f As New frmPurchaseSearch()
    ...
    e As EventArgs
        ...
        ' 🔥 الجديد
        Dim affected = _revaluationService.GetAffectedLedgers(detailIDs)

        If affected Is Nothing OrElse affected.Count = 0 Then
            MessageBox.Show("لا توجد عمليات متأثرة.")
            Exit Sub
        End If

        ' 🔥 تحويل إلى DataTable
        Dim dt As DataTable = ConvertToDataTable(affected)
    ...
    Private Function GetChangedDetailIDs() As List(Of Integer)
        ...
        Dim dt As DataTable = TryCast(dgvAdjustResult.DataSource, DataTable)
        If dt Is Nothing OrElse dt.Rows.Count = 0 Then Return result

        If Not dt.Columns.Contains("SourceDetailID") Then Return result
        ...
        For Each r As DataRow In dt.Rows
            ...
            Dim qtyDelta As Decimal = 0D
            Dim costDelta As Decimal = 0D

            If dt.Columns.Contains("QtyDelta") AndAlso Not IsDBNull(r("QtyDelta")) Then
                qtyDelta = Convert.ToDecimal(r("QtyDelta"))
            End If

            If dt.Columns.Contains("CostDelta") AndAlso Not IsDBNull(r("CostDelta")) Then
                costDelta = Convert.ToDecimal(r("CostDelta"))
            End If

            If qtyDelta <> 0D OrElse costDelta <> 0D Then
                Dim id As Integer = Convert.ToInt32(r("SourceDetailID"))
   ...
   r.Table Is CType(dgvProduced.DataSource, DataTable) Then
            ...
            row("SourceDetailID") = ToInt(tr("DetailID"))
            row("ProductID") = ToInt(tr("ProductID"))
            row("ProductCode") = tr("ProductCode").ToString()
            row("ProductName") = tr("ProductName").ToString()
    ...
    Private Sub BuildAdjustmentDelta_CUT()
        ...
        '========================================
        Dim dtTrans As DataTable =
        _revaluationService.GetTransactionDetailsByDetailIDs_CUT(detailIDs)
        ...
        Dim cuttingProductID As Integer = 0
        Dim cuttingProductCode As String = ""
        Dim cuttingProductName As String = ""

        '========================================
        ' استخراج صف الخارج (مرة واحدة)
        '========================================
        Dim trOutGlobal As DataRow = Nothing

        For Each tr As DataRow In dtTrans.Rows
            If ToDec(tr("OutQty")) > 0 Then
                trOutGlobal = tr
                Exit For
            End If
        Next

        If trOutGlobal IsNot Nothing Then
            cuttingSourceID = ToInt(trOutGlobal("DetailID"))
            cuttingOldCost = ToDec(trOutGlobal("UnitCost"))
            cuttingProductID = ToInt(trOutGlobal("ProductID"))
            cuttingProductCode = trOutGlobal("ProductCode").ToString()
            cuttingProductName = trOutGlobal("ProductName").ToString()
        End If
        '========================================
        ' 4) Dictionary
        '========================================
        Dim dict As New Dictionary(Of Long, List(Of DataRow))
        ...
        For Each tr As DataRow In dtTrans.Rows
            ...
            If Not dict.ContainsKey(key) Then
                dict.Add(key, New List(Of DataRow))
            End If
        ...
        For Each r As DataRow In dt.Rows
            ...
            Dim trList As List(Of DataRow) = dict(sourceID)

            Dim trIn As DataRow = Nothing
            Dim trOut As DataRow = Nothing

            For Each trX As DataRow In trList

                Dim outQty As Decimal = ToDec(trX("OutQty"))
                Dim inQty As Decimal = ToDec(trX("InQty"))
                ...
                If outQty > 0 Then
                    ...
                    ' 🟢 الداخل
                ElseIf inQty > 0 Then
                    If trIn Is Nothing Then
                        trIn = trX
                    End If
                End If

            Next
            If trIn Is Nothing Then Continue For

            Dim oldQty As Decimal = ToDec(trIn("InQty"))
            Dim oldCost As Decimal = ToDec(trIn("UnitCost"))
            '========================================
            ' MIX؟
            '========================================
            ...
            Else
                ' 🟢 عادي → الجريد يعدل بالقطعة لكن المقارنة يجب أن تكون بالحجم
                Dim pieceQty As Decimal = ToDecSafe(r, "Quantity")

                Dim length As Decimal = ToDecSafe(r, "Length_cm")
                Dim width As Decimal = ToDecSafe(r, "Width_cm")
                Dim height As Decimal = ToDecSafe(r, "Height_cm")

                Dim pieceVolume As Decimal = length * width * height / 1000000D

                newQty = pieceQty

                ' oldQty جاء من CostLedger.InQty وهو بالحجم
                oldCompare = oldQty
                newCompare = pieceQty * pieceVolume

                ' الخارج دائمًا بالحجم
                totalOldVolume += oldQty
                totalNewVolume += newCompare
            End If
            ...
            row("SourceDetailID") = ToInt(trIn("DetailID"))
            row("ProductID") = ToInt(trIn("ProductID"))
            row("ProductCode") = trIn("ProductCode").ToString()
            row("ProductName") = trIn("ProductName").ToString()
        ...
        If cuttingSourceID <> 0 Then
            ...
            If deltaOut <> 0D Then
                ...
                row("ProductID") = cuttingProductID
                row("ProductCode") = cuttingProductCode
                row("ProductName") = cuttingProductName
                row("OldQty") = oldOut
                row("NewQty") = newOut
    ...
    'SCP
    Private Sub LoadWasteDocumentForRevaluation(wasteID As Integer)
...
", con)
                ...
                If dt.Rows.Count = 0 Then
                    Throw New Exception("لم يتم العثور على مستند السكراب")
                End If
                ...
                ' حفظ المرجع
                '========================================
                _referenceTransactionID = wasteID
                _referenceType = "SCRAP"

                '========================================
                ' تعبئة الحقول (READ ONLY)
                '========================================
                txtScrapCode.Text = Convert.ToString(r("WasteCode"))

                dtpPeriodStart.Value = CDate(r("PeriodStartDate"))
                dtpPeriodEnd.Value = CDate(r("PeriodEndDate"))

                txtWastePercent.Text = CDec(r("WastePercent")).ToString("0.00")
                txtWasteAvgCost.Text = CDec(r("WasteAvgCost")).ToString("0.000")

                txtTotalWasteVolume.Text = CDec(r("TotalWasteVolume_m3")).ToString("0.000")
                txtTotalWasteWeight.Text = CDec(r("TotalWasteWeight_kg")).ToString("0.000")
                txtTotalWasteValue.Text = CDec(r("TotalWasteValue")).ToString("0.000")
                txtTotalScrapValue.Text = CDec(r("TotalScrapValue")).ToString("0.000")
...
", con)
                ...
                For Each dr As DataRow In dtDet.Rows
                    ...
                    Dim i = dgvCalculations.Rows.Add()
                    Dim row = dgvCalculations.Rows(i)

                    row.Cells("colDetailID").Value = CInt(dr("WasteDetailID"))
                    row.Cells("colConsumption").Value = CDec(dr("ConsumptionQty"))
                    row.Cells("colWastePercent").Value = CDec(dr("WastePercent"))
                    row.Cells("colWasteQTY").Value = CDec(dr("WasteQty"))
                    row.Cells("colProductAvailableQty").Value = CDec(dr("AvailableQty"))
                    row.Cells("colProductDensity").Value = CDec(dr("Density"))

                    ' المحسوبة (نحسبها مرة للعرض)
    ...
    End Sub
    Private Sub ApplyCancelMode()

        IsCancelMode = True

        '========================================
        ' الحقول (نصفر فقط القيم المحاسبية)
        '========================================
        txtWasteAvgCost.Text = "0"
        ...
        ' الجريد
        '========================================
        For Each row As DataGridViewRow In dgvCalculations.Rows

            If row.IsNewRow Then Continue For

            row.Cells("colWastePercent").Value = 0D
            row.Cells("colWasteQTY").Value = 0D
            row.Cells("colWasteValue").Value = 0D
            row.Cells("colTotalWaste").Value = 0D
    ...
    Private Sub SetFormReadOnly()
        ...
        For Each ctrl As Control In Me.Controls
            DisableControlRecursive(ctrl)
        Next

    End Sub

    Private Sub DisableControlRecursive(ctrl As Control)
    ...
    Private Sub ComputeTotalWaste(row As DataGridViewRow)
        ...
        Dim wasteQty As Decimal = 0D
        Dim density As Decimal = 0D

        If Not IsDBNull(row.Cells("colWasteQTY").Value) Then
            wasteQty = CDec(row.Cells("colWasteQTY").Value)
        End If

        If Not IsDBNull(row.Cells("colProductDensity").Value) Then
            density = CDec(row.Cells("colProductDensity").Value)
        End If

        ' مثال حساب (عدل حسب منطقك الحقيقي)
        Dim totalWaste As Decimal = wasteQty * density

        row.Cells("colTotalWaste").Value = totalWaste

        ' القيمة (لو عندك سعر)
        If row.Cells("colWasteValue") IsNot Nothing Then
            row.Cells("colWasteValue").Value = 0D ' لأننا في review فقط
        End If
    ...
    Private Function ConvertToDataTable(list As List(Of AvgCostData)) As DataTable

        Dim dt As DataTable = CreateAffectedOperationsTable()

        For Each l In list

            Dim row = dt.NewRow()

            row("LedgerID") = l.LedgerID
            row("TransactionID") = l.TransactionID
            row("SourceDetailID") = l.SourceDetailID

            row("ProductID") = l.ProductID
            row("BaseProductID") = l.BaseProductID
            row("StoreID") = l.StoreID
            row("OperationTypeID") = l.OperationTypeID

            row("LocalOldQty") = l.LocalOldQty
            row("OldQty") = l.OldQty
            ...
            row("NewQty") = l.NewQty
            row("LocalNewQty") = l.LocalNewQty

            row("OldAvgCost") = l.OldAvgCost
            row("InUnitCost") = l.InUnitCost
            row("OutUnitCost") = l.OutUnitCost
            row("NewAvgCost") = l.NewAvgCost

            row("LedgerSequence") = l.LedgerSequence
            row("SourceLedgerID") = If(l.SourceLedgerID.HasValue, l.SourceLedgerID.Value, DBNull.Value)

            row("SupersededByLedgerID") = DBNull.Value
            row("RootTransactionID") = DBNull.Value

            row("PostingDate") = l.PostingDate
